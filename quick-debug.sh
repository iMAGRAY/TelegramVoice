...
echo
echo "2. Проверка портов (ss):"
ss -tlnp | grep -E "(8080|3000)" || echo "Порты не найдены"

echo
echo "2.1 Проверка портов (lsof):"
lsof -i:8080 2>/dev/null || echo "Порт 8080 никто не слушает"
lsof -i:3000 2>/dev/null || echo "Порт 3000 никто не слушает"
