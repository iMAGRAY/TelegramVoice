  ...
  private handleMessage(message: СообщениеСервера) {
    switch (message.тип) {
      ...
      case 'участники-комнаты-обновлены':
        console.log(`👥 ${this.имя} видит участников: [${message.участники.map((у: any) => у.имя).join(', ')}]`);
        break;
      
      case 'комнаты-обновлены':
        console.log(`📋 ${this.имя} получил список комнат: [${message.комнаты.map((к: any) => к.название).join(', ')}]`);
        break;
    }
  }
