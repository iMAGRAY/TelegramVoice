...
export class WebSocketServer {
  ...
  private присоединитьсяККомнате(ws: WSConnection, комнатаId: string, пароль?: string) {
    ...
    // ИСПРАВЛЕНИЕ: Уведомляем ДРУГИХ участников о присоединении нового пользователя
  ...
  private покинутьКомнату(ws: WSConnection, комнатаId: string) {
    ...
    комната.участники = комната.участники.filter((id: string) => id !== ws.userId);
  ...
  private обновитьУчастниковКомнаты(комнатаId: string) {
    ...
    const участники = комната.участники
      .map((id: string) => this.пользователи.get(id))
      .filter((п: Пользователь | undefined): п is Пользователь => п !== undefined);

    console.log(`👥 Обновление участников комнаты ${комната.название}: [${участники.map((у: Пользователь) => у.имя).join(', ')}]`);
  ...
  private отправитьВКомнату(комнатаId: string, сообщение: СообщениеСервера, исключить?: string) {
    ...
    комната.участники.forEach((userId: string) => {
      if (userId !== исключить) {
