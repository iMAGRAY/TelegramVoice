...
impl СерверСостояние {
    ...
    pub async fn отправить_пользователю(&self, пользователь_id: &str, сообщение: ОтветСервера) {
        if let Some(sender) = self.websockets.get(пользователь_id) {
            ...
            error!("WebSocket соединение для пользователя {} не найдено", пользователь_id);
        }
    }
...
async fn обработать_соединение(
    ...
    состояние.websockets.insert(connection_id.clone(), tx);
    ...
    while let Some(msg) = ws_receiver.next().await {
        match msg {
            Ok(Message::Text(text)) => {
                let current_user_id = пользователь_id.clone().unwrap_or_else(|| connection_id.clone());
                if let Err(e) = обработать_сообщение(&text, &current_user_id, &connection_id, &состояние, &mut пользователь_id).await {
                    error!("Ошибка обработки сообщения: {}", e);
                    состояние.отправить_пользователю(&current_user_id, ОтветСервера::Ошибка {
    ...
    состояние.websockets.remove(&connection_id);
    
    if let Some(user_id) = пользователь_id {
        if let Some(пользователь) = состояние.пользователи.get(&user_id) {
        ...
        состояние.пользователи.remove(&user_id);
        info!("Пользователь {} отключился", user_id);
    } else {
        info!("Неидентифицированное соединение {} отключилось", connection_id);
    }
}
...
async fn обработать_сообщение(
    ...
    connection_id: &str,
    состояние: &СерверСостояние,
    пользователь_id: &mut Option<String>,
) -> Result<()> {
    ...
    match message_type {
        "подключение" => {
            if let Some(данные) = json.get("данные") {
                ...
                состояние.пользователи.insert(current_user_id.to_string(), пользователь.clone());
                
                // Отправляем список комнат новому пользователю
                let комнаты = состояние.получить_комнаты();
                состояние.отправить_пользователю(current_user_id, ОтветСервера::КомнатыОбновлены { комнаты }).await;
            }
        }
    ...
    match сообщение {
        СообщениеВебСокета::Присоединиться { пользователь } => {
            info!("Пользователь {} присоединился с ID {}", пользователь.имя, пользователь.id);
            ...
            *пользователь_id = Some(user_id.clone());
            ...
            состояние.пользователи.insert(user_id.clone(), пользователь.clone());
            ...
            // Отправляем список комнат новому пользователю
            let комнаты = состояние.получить_комнаты();
            состояние.отправить_пользователю(&user_id, ОтветСервера::КомнатыОбновлены { комнаты }).await;
        }
        ...
        СообщениеВебСокета::СоздатьКомнату { название, создатель, максимум_участников, приватная, пароль } => {
            match состояние.создать_комнату(название, создатель, максимум_участников, приватная, пароль) {
                Ok(комната) => {
                    ...
                    if let Some(пользователь) = состояние.пользователи.get(current_user_id) {
                        состояние.отправить_пользователю(current_user_id, ОтветСервера::ПрисоединилсяККомнате {
        ...
        СообщениеВебСокета::WebRTCСигнал { от, к, комната, данные } => {
            // Логируем WebRTC сигнал для отладки
            info!("WebRTC сигнал: от {} к {} в комнате {}", от, к, комната);
            if let Some(signal_type) = данные.get("type") {
                info!("  Тип сигнала: {}", signal_type);
            }
            ...
            info!("WebRTC сигнал отправлен пользователю {}", к);
        }
        ...
        СообщениеВебСокета::ПолучитьПользователейКомнаты { комната_id } => {
            let участники = состояние.получить_участников_комнаты(&комната_id);
            состояние.отправить_пользователю(current_user_id, ОтветСервера::УчастникиКомнатыОбновлены {
                комната_id,
                участники,
            }).await;
        }
...
fn создать_предустановленные_комнаты(состояние: &СерверСостояние) {
    // Список предустановленных комнат
    let комнаты = vec![
        ("general", "Общий", 50),
        ("important", "Комната важных переговоров", 10),
        ("channel1", "Канал 1", 20),
        ("channel2", "Канал 2", 20),
        ("channel3", "Канал 3", 20),
    ];
    
    for (id, название, максимум) in комнаты {
        let комната = Комната {
            id: id.to_string(),
            название: название.to_string(),
            создатель: "system".to_string(),
            участники: Vec::new(),
            максимум_участников: максимум,
            создана: Utc::now(),
            активна: true,
            приватная: false,
            пароль: None,
        ...
        состояние.комнаты.insert(комната.id.clone(), комната.clone());
        info!("Создана предустановленная комната: {}", название);
    }
}
...
async fn main() -> Result<()> {
    ...
    // Создаем предустановленные комнаты
    создать_предустановленные_комнаты(&состояние);
