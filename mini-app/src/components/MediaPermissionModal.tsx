...
import { PermissionBlockedHelp } from './PermissionBlockedHelp';
...
export const MediaPermissionModal: React.FC<MediaPermissionModalProps> = ({
  ...
  const [показать_помощь, setПоказать_помощь] = useState(false);
      ...
      if (требуется_микрофон && статус_микрофона !== 'разрешено') {
        ...
        if (!результат) {
          ...
          if (статус_микрофона === 'отклонено') {
            setОшибка('🔒 Браузер заблокировал доступ к микрофону. Нажмите "Настройки браузера" для получения подробных инструкций по разблокировке.');
          }
        }
      }
  ...
  const обработать_настройки_браузера = () => {
    setПоказать_помощь(true);
  };

  const принудительный_сброс = async () => {
    console.log('[MediaPermissionModal] Принудительный сброс и повторный запрос');
    setОшибка(null);
    
    // Сбрасываем статус
    сбросить_статус_микрофона();
    
    // Небольшая задержка для Telegram Mini App
    ...
    try {
      ...
      if (результат) {
        ...
        setОшибка('Не удалось получить разрешение даже после сброса. Попробуйте перезагрузить страницу.');
      }
    } catch (error) {
      console.error('[MediaPermissionModal] Ошибка при принудительном запросе:', error);
      setОшибка('Произошла ошибка при принудительном запросе разрешений.');
    } finally {
      setЗагружается(false);
    }
  };
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-[var(--bg-primary)] rounded-lg shadow-lg max-w-md w-full p-6">
        ...
        {((требуется_микрофон && статус_микрофона === 'отклонено') || 
          ...
          <div className="mb-4 p-3 bg-[var(--warning)] bg-opacity-10 border border-[var(--warning)] rounded-lg">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-[var(--warning)] text-sm font-medium">Разрешения заблокированы</p>
                <p className="text-[var(--text-secondary)] text-xs mt-1">
                  Браузер запомнил ваш выбор и больше не показывает диалог. Используйте кнопку "Настройки браузера" для разблокировки.
                </p>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-3">
          ...
          {/* Кнопки для заблокированных разрешений */}
          {((требуется_микрофон && статус_микрофона === 'отклонено') || 
            (требуется_камера && статус_камеры === 'отклонено')) && (
            <div className="space-y-2">
              <button
                onClick={принудительный_сброс}
                disabled={загружается}
                className="w-full px-4 py-3 bg-[var(--warning)] hover:bg-opacity-90 disabled:opacity-50 text-white rounded-lg transition-colors font-medium"
              >
                {загружается ? 'Сбрасываем...' : '🔄 Принудительный сброс'}
              </button>
              <button
      ...
      <PermissionBlockedHelp
        открыт={показать_помощь}
        на_закрыть={() => setПоказать_помощь(false)}
      />
    </div>
  );
};