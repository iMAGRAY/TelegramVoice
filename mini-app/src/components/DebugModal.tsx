...
export const DebugModal: React.FC<DebugModalProps> = ({ открыт, на_закрыть }) => {
  ...
  useEffect(() => {
    ...
    let лог_буфер: LogEntry[] = [];
    let таймер_обновления: NodeJS.Timeout | null = null;
    ...
    const добавить_лог = (level: LogEntry['level'], args: any[]) => {
      ...
      лог_буфер.push(новый_лог);
      
      // Дебаунсинг обновлений для снижения частоты перерисовок
      if (таймер_обновления) {
        clearTimeout(таймер_обновления);
      }
      
      таймер_обновления = setTimeout(() => {
        setЛоги(prev => {
          const новые_логи = [...prev, ...лог_буфер];
          лог_буфер = [];
          return новые_логи.slice(-1000); // Ограничиваем до 1000 записей
        });
        таймер_обновления = null;
      }, 200); // Обновляем максимум каждые 200мс
    };
