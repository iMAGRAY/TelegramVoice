...
export const SimpleVoiceRoom: React.FC<VoiceRoomProps> = ({
  ...
  // Используем JSON.stringify для участников чтобы сравнивать по содержимому, а не по ссылке
  const участники_ids = участники.map(у => у.id).sort().join(',');
  ...
  }, [локальный_поток, участники_ids, текущий_пользователь.id, безопасно_подключиться_к_пользователю]);
