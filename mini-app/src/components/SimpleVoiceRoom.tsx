...
export const SimpleVoiceRoom: React.FC<VoiceRoomProps> = ({
  ...
  const обработать_разрешение_получено = () => {
    ...
    if (локальный_поток) {
      console.log('[SimpleVoiceRoom] Микрофон уже получен, синхронизируем состояние');
      синхронизировать_состояние_микрофона();
      return;
    }
  ...
  // Инициализация микрофона при входе в комнату - ТОЛЬКО ОДИН РАЗ
  useEffect(() => {
    console.log('[SimpleVoiceRoom] Проверка инициализации микрофона:', {
      разрешения_проверены,
      статус_микрофона,
      поддерживается,
      уже_есть_поток: !!локальный_поток
    });
    
    // КРИТИЧНО: Не создаем микрофон если он уже есть
    if (локальный_поток) {
      console.log('[SimpleVoiceRoom] Микрофон уже получен, пропускаем инициализацию');
      return;
    }
  ...
  }, [разрешения_проверены, статус_микрофона, локальный_поток, получить_микрофон, очистить, синхронизировать_состояние_микрофона]);
