...
export const SimpleVoiceRoom: React.FC<VoiceRoomProps> = ({
  ...
  const безопасно_подключиться_к_пользователю = useCallback((пользователь_id: string) => {
    ...
    if (!локальный_поток) {
      console.warn(`[SimpleVoiceRoom] ⚠️ ПРОБЛЕМА: Ожидание локального потока перед подключением к ${пользователь_id}`);
  ...
  useEffect(() => {
    ...
    if (разрешения_проверены && статус_микрофона === 'разрешено') {
      ...
      получить_микрофон().then((поток) => {
        console.log('[SimpleVoiceRoom] ✅ Микрофон получен автоматически:', {
          треки: поток?.getTracks().length || 0,
          активен: поток?.active,
          id: поток?.id
        });
