...
export const SimpleVoiceRoom: React.FC<VoiceRoomProps> = ({
  ...
  useEffect(() => {
    // Ждем завершения проверки совместимости
    if (!webrtc_диагностика.совместимость_проверена) {
      return;
    }
    
    // Если WebRTC не поддерживается, не пытаемся получить микрофон
    if (!webrtc_диагностика.поддерживается) {
      console.log('[SimpleVoiceRoom] WebRTC не поддерживается, пропускаем инициализацию микрофона');
      return;
    }
