...
export const SimpleVoiceRoom: React.FC<VoiceRoomProps> = ({
    ...
    const unsubscribe2 = подписаться('присоединился-к-комнате', (данные: any) => {
      if (данные.комната?.id === комната.id && данные.пользователь?.id !== текущий_пользователь.id) {
        ...
        setTimeout(() => {
          ...
          подключиться_к_пользователю(данные.пользователь.id);
  ...
  }, [socket, комната.id, текущий_пользователь.id, подключиться_к_пользователю, подписаться, микрофон_включен, удалить_peer]);

  // Инициализация микрофона при входе в комнату - КРИТИЧНО: должен быть получен ДО подключения к другим пользователям
  useEffect(() => {
    ...
    if (разрешения_проверены && статус_микрофона === 'разрешено') {
      console.log('[SimpleVoiceRoom] Автоматически получаем микрофон');
      получить_микрофон().then(() => {
        ...
        if (socket && socket.получить_пользователей_комнаты) {
          console.log('[SimpleVoiceRoom] Запрашиваем список участников комнаты');
      ...
      console.log('[SimpleVoiceRoom] Ждем разрешения микрофона');
    }
  ...
  }, [разрешения_проверены, статус_микрофона, локальный_поток, получить_микрофон, очистить, синхронизировать_состояние_микрофона, socket, комната.id]);
