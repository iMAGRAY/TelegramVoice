...
export const ICEStatus: React.FC<ICEStatusProps> = ({ показать_подробности = false }) => {
  ...
  const проверить_соединение = async () => {
    try {
      ...
      console.error('Ошибка диагностики ICE:', error);
      setСостояние('плохо');
      setРекомендации(['Не удалось проверить ICE серверы']);
    }
  };
