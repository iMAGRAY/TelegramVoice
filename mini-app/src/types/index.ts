...
export interface СообщениеВебСокета {
  тип: 'подключение' | 'присоединиться-к-комнате' | 'покинуть-комнату' | 'создать-комнату' | 'получить-комнаты' | 'получить-пользователей-комнаты' | 
       'offer' | 'answer' | 'ice-candidate' | 'пользователи-обновлены' | 'комнаты-обновлены' | 
       'микрофон-переключен' | 'говорит' | 'ошибка' | 'сообщение';
  данные: any;
  от?: string;
  к?: string;
  комната?: string;
