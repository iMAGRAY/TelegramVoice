...
export const useWebRTC = ({
  ...
  const обработать_сигнал = useCallback((сообщение: any) => {
    ...
    if (тип_сигнала === 'offer') {
      // ИСПРАВЛЕНИЕ: Проверяем наличие локального потока и создаем его если нужно
      if (!локальный_поток_ref.current) {
        console.warn(`[WebRTC] Получен offer от ${от}, но нет локального потока - откладываем обработку`);
        // Откладываем обработку на небольшое время
        setTimeout(() => {
          console.log(`[WebRTC] Повторная попытка обработки offer от ${от}`);
          обработать_сигнал(сообщение);
        }, 1000);
        return;
      }
