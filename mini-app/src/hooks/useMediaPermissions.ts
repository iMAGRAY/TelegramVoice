...
export const useMediaPermissions = (): UseMediaPermissionsResult => {
  ...
  useEffect(() => {
    ...
    // ИСПРАВЛЕНИЕ: Убираем глобальный флаг, каждый компонент должен проверять статус
    // Добавляем небольшую задержку чтобы избежать гонки состояний
    const таймер = setTimeout(() => {
      if (монтирован && поддерживается) {
        проверить_статус();
      }
    }, 100);
  ...
  }, [поддерживается, проверить_статус]); // Добавляем зависимости для корректной работы
