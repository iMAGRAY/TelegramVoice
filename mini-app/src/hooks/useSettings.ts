...
export const useSettings = () => {
  ...
  useEffect(() => {
    console.log(`[useSettings] 🔄 Начинаем загрузку настроек...`);
    try {
      const сохраненные_настройки = localStorage.getItem(КЛЮЧ_НАСТРОЕК);
      console.log(`[useSettings] 📂 Сохраненные настройки из localStorage:`, сохраненные_настройки);
      
      if (сохраненные_настройки) {
        const parsed = JSON.parse(сохраненные_настройки);
        console.log(`[useSettings] 📋 Распарсенные настройки:`, parsed);
        setНастройки({
        ...
        console.log(`[useSettings] ✅ Настройки загружены из localStorage`);
      } else {
        console.log(`[useSettings] 🆕 Используем настройки по умолчанию`);
      }
    } catch (error) {
      console.error('[useSettings] ❌ Ошибка загрузки настроек:', error);
    } finally {
      console.log(`[useSettings] 🏁 Завершаем загрузку настроек - устанавливаем загружено=true`);
      setЗагружено(true);
    }
  }, []);

  // АВАРИЙНЫЙ таймер для принудительной загрузки настроек
  useEffect(() => {
    const emergencyTimer = setTimeout(() => {
      if (!загружено) {
        console.warn(`[useSettings] 🚨 АВАРИЙНАЯ ЗАГРУЗКА - настройки не загрузились за 3 секунды!`);
        setЗагружено(true);
      }
    }, 3000); // 3 секунды
