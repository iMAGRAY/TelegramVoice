...
export const useSocket = ({
  ...
  // ИСПРАВЛЕНИЕ: Используем ID пользователя вместо объекта для стабильности зависимостей
  const пользователь_id = пользователь?.id;
  ...
  useEffect(() => {
    ...
    if (пользователь_id && !socket) {
      console.log(`[useSocket] 🚀 Инициализация автоматического подключения для пользователя ${пользователь_id}`);
      подключиться();
    } else if (пользователь_id && socket) {
      console.log(`[useSocket] ℹ️ Пользователь есть, сокет есть (состояние: ${socket.readyState})`);
    } else if (!пользователь_id) {
  ...
  }, [пользователь_id, подключиться]);

  // Обработка переподключения при восстановлении соединения
  useEffect(() => {
    const обработать_онлайн = () => {
      if (пользователь_id && socket && socket.readyState !== WebSocket.OPEN) {
        подключиться();
      }
    };
  ...
  }, [пользователь_id, socket, подключиться]);
