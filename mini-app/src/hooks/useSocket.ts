  ...
  const подключиться = useCallback(async () => {
    ...
    try {
      ...
      новый_socket.onmessage = (event) => {
        try {
          const сообщение = JSON.parse(event.data) as СообщениеВебСокета;
          
          // Вызываем обработчики для типа сообщения
          const handlers = обработчики.current.get(сообщение.тип);
          if (handlers) {
            handlers.forEach(handler => {
              handler(сообщение.данные);
            });
          }
          ...
          switch (сообщение.тип) {
            case 'пользователи-обновлены':
              на_обновление_пользователей?.(сообщение.данные as Пользователь[]);
              break;
            case 'комнаты-обновлены':
              на_обновление_комнат?.(сообщение.данные as Комната[]);
              break;
            case 'ошибка':
              console.error('Ошибка от сервера:', сообщение.данные);
              на_ошибку?.(сообщение.данные as string);
              break;
          }
        } catch (error) {
